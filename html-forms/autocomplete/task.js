class Autocomplete {
  ...
  getMatches( text ) {
     ...
     // let a = Array.from(this.input).filter( (item) => { return item.text.includes(this.searchInput.value) })   
     // let result =  a.map(function(item) {return {text:item.text,value:item.value}})   
     // return result

    return  Array.from(this.input).filter( (item) => { return item.text.includes(this.searchInput.value) }).map(function(item) {return {text:item.text,value:item.value}})   
